#! /bin/bash
# This script is run on gateway-b when it boots up

## NAT traffic going to the internet
route add default gw 172.27.27.2
iptables -t nat -A POSTROUTING -o eth1 -j MASQUERADE

## Bind the IP address of original local server to the interface
ip addr add 10.2.0.99/16 dev eth0

## Redirect to cloud with Destination NAT
iptables -t nat -A PREROUTING -p tcp -d 10.2.0.99 --dport 8080 -j DNAT --to 10.3.0.3:8080

## Allow VPN traffic to the cloud
iptables -t nat -I POSTROUTING -d 10.3.0.3 -j ACCEPT

## Iptables rules (strict firewall)
### Accept IKE and esp traffic from/to the cloud
iptables -A INPUT -i eth1 -p udp -s 172.28.28.28 --sport 500 -d 172.27.27.27 --dport 500 -j ACCEPT
iptables -A INPUT -i eth1 -p udp -s 172.28.28.28 --sport 4500 -d 172.27.27.27 --dport 4500 -j ACCEPT
iptables -A INPUT -i eth1 -p esp -s 172.28.28.28 -d 172.27.27.27 -j ACCEPT
iptables -A OUTPUT -o eth1 -p udp -s 172.27.27.27 --sport 500 -d 172.28.28.28 --dport 500 -j ACCEPT
iptables -A OUTPUT -o eth1 -p udp -s 172.27.27.27 --sport 4500 -d 172.28.28.28 --dport 4500 -j ACCEPT
iptables -A OUTPUT -o eth1 -p esp -s 172.27.27.27 -d 172.28.28.28 -j ACCEPT
### Drop everything else (including Internet traffic)
iptables -A INPUT -j DROP
iptables -A OUTPUT -j DROP

## Stop internet traffic
iptables -A FORWARD -j DROP

## Save the iptables rules
iptables-save > /etc/iptables/rules.v4
ip6tables-save > /etc/iptables/rules.v6

## Certificates
cat > /etc/ipsec.d/cacerts/caCert.pem <<EOL
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOL

cat > /etc/ipsec.d/cacerts/intCaCert.pem <<EOL
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOL

cat > /etc/ipsec.d/certs/siteBCert.pem <<EOL
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOL

cat > /etc/ipsec.d/certs/cloudCert.pem <<EOL
-----BEGIN CERTIFICATE-----

8KJL7VjyN+gjUFGu5LQwV4GFo4QgL/VI8mj9
-----END CERTIFICATE-----
EOL

cat > /etc/ipsec.d/private/siteBKey.pem <<EOL
-----BEGIN EC PRIVATE KEY-----
MIGkAgEBBDA/cJsccQYJvwAgndNG0auYMkrk23x0F/jBOGe3S798j3NBFNkDSgV/
J9IjEqk1fxigBwYFK4EEACKhZANiAAScli6NTC3qhx0iK1bmKmoOu8CoZPxbJLXZ
uHenEEM8GwFPS17UJkj8ab9hBnH5K2s/QVrVUofnRuDGTfh6MEnS923n9lpAFUTC
Kt+9wlMboub2PHkc7jHM49scN72wxRw=
-----END EC PRIVATE KEY-----
EOL

## Certificate revocation lists
cp /crls/* /etc/ipsec.d/crls/

## Ipsec config
FIND_FILE="/etc/ipsec.secrets"
FIND_STR=": ECDSA siteBKey.pem"
if [ `grep -c "$FIND_STR" $FIND_FILE` == '0' ];then
    echo "$FIND_STR" >> $FIND_FILE
fi

cat > /etc/ipsec.conf <<EOL
conn b-to-cloud
        keyexchange=ikev2
        leftfirewall=yes
        rightfirewall=yes
        left=172.27.27.27
        leftsubnet=10.2.0.0/16
        leftid=172.27.27.27
        leftcert=siteBCert.pem
        leftid="C=FI, O=CSE4300, CN=CSE4300 Site B 172.27.27.27"
        leftca="C=FI, O=CSE4300, CN=CSE4300 Root CA"
        right=172.28.28.28
        rightsubnet=10.3.0.0/16
        rightcert=cloudCert.pem
        rightid="C=FI, O=CSE4300, CN=CSE4300 Cloud 172.28.28.28"
        rightca="C=FI, O=CSE4300, CN=CSE4300 Root CA"
        ike=aes256gcm16-prfsha384-ecp384!
        esp=aes256gcm16-ecp384!
        auto=route
        dpdaction=hold
EOL

## Start ipsec for updates to take effect
ipsec start

# Prevent the container from exiting
while true; do
    sleep 60
done

#! /bin/bash
# This script is run on gateway-s when it boots up

## Traffic going to the internet
route add default gw 172.30.30.2

## NAT Masquerade for eth1
iptables -t nat -A POSTROUTING -o eth1 -j MASQUERADE

## Iptables rules (strict firewall)
### Accept IKE and esp traffic from/to the clients
iptables -A INPUT -i eth1 -p udp -s 172.16.16.16 --sport 500 -d 172.30.30.30 --dport 500 -j ACCEPT
iptables -A INPUT -i eth1 -p udp -s 172.16.16.16 --sport 4500 -d 172.30.30.30 --dport 4500 -j ACCEPT
iptables -A INPUT -i eth1 -p esp -s 172.16.16.16 -d 172.30.30.30 -j ACCEPT
iptables -A INPUT -i eth1 -p udp -s 172.18.18.18 --sport 500 -d 172.30.30.30 --dport 500 -j ACCEPT
iptables -A INPUT -i eth1 -p udp -s 172.18.18.18 --sport 4500 -d 172.30.30.30 --dport 4500 -j ACCEPT
iptables -A INPUT -i eth1 -p esp -s 172.18.18.18 -d 172.30.30.30 -j ACCEPT
iptables -A OUTPUT -o eth1 -p udp -s 172.30.30.30 --sport 500 -d 172.16.16.16 --dport 500 -j ACCEPT
iptables -A OUTPUT -o eth1 -p udp -s 172.30.30.30 --sport 4500 -d 172.16.16.16 --dport 4500 -j ACCEPT
iptables -A OUTPUT -o eth1 -p esp -s 172.30.30.30 -d 172.16.16.16 -j ACCEPT
iptables -A OUTPUT -o eth1 -p udp -s 172.30.30.30 --sport 500 -d 172.18.18.18 --dport 500 -j ACCEPT
iptables -A OUTPUT -o eth1 -p udp -s 172.30.30.30 --sport 4500 -d 172.18.18.18 --dport 4500 -j ACCEPT
iptables -A OUTPUT -o eth1 -p esp -s 172.30.30.30 -d 172.18.18.18 -j ACCEPT
### Drop everything else (including Internet traffic)
iptables -A INPUT -j DROP
iptables -A OUTPUT -j DROP

## Save the iptables rules
iptables-save > /etc/iptables/rules.v4
ip6tables-save > /etc/iptables/rules.v6

## Certificates
cat > /etc/ipsec.d/cacerts/caCert.pem <<EOL
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOL

cat > /etc/ipsec.d/cacerts/intCaCert.pem <<EOL
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOL

cat > /etc/ipsec.d/certs/siteACert.pem <<EOL
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOL

cat > /etc/ipsec.d/certs/siteBCert.pem <<EOL
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOL

cat > /etc/ipsec.d/certs/cloudCert.pem <<EOL
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
EOL

cat > /etc/ipsec.d/private/cloudKey.pem <<EOL
-----BEGIN EC PRIVATE KEY-----
MIGkAgEBBDAxLt5tXh4sYn5OtW96vNlmIkk6TS3VdytHVwGfTuwZO+6DWEDG65nF
0K36QlKkp8KgBwYFK4EEACKhZANiAARAAbJohiFABaKbkRvJ3zuAbHl3EiWCTxob
xKVV+0FTc72OqV4KJJkC0m60Ky2MqhStQGszK4nnLr7XlBzccBJFeIMkV4mii8ws
8bjYnvcifCmIVg+Hy3UZUEE5Zh/qmyM=
-----END EC PRIVATE KEY-----
EOL

## Certificate revocation lists
cp /crls/* /etc/ipsec.d/crls/

## Ipsec config
FIND_FILE="/etc/ipsec.secrets"
FIND_STR=": ECDSA cloudKey.pem"
if [ `grep -c "$FIND_STR" $FIND_FILE` == '0' ];then
    echo "$FIND_STR" >> $FIND_FILE
fi

cat > /etc/ipsec.conf <<EOL
conn %default
        keyexchange=ikev2
        leftfirewall=yes
        rightfirewall=yes
        left=172.30.30.30
        leftsubnet=10.3.0.0/16
        leftcert=cloudCert.pem
        leftid="C=FI, O=CSE4300, CN=CSE4300 Cloud 172.30.30.30"
        leftca="C=FI, O=CSE4300, CN=CSE4300 Root CA"
        rightca="C=FI, O=CSE4300, CN=CSE4300 Root CA"
        ike=aes256gcm16-prfsha384-ecp384!
        esp=aes256gcm16-ecp384!
        auto=route
        dpdaction=hold
conn cloud-to-a
        also=%default
        right=172.16.16.16
        rightsubnet=10.1.0.0/16
        rightcert=siteACert.pem
        rightid="C=FI, O=CSE4300, CN=CSE4300 Site A 172.16.16.16"
conn cloud-to-b
        also=%default
        right=172.18.18.18
        rightsubnet=10.2.0.0/16
        rightcert=siteBCert.pem
        rightid="C=FI, O=CSE4300, CN=CSE4300 Site B 172.18.18.18"
EOL

## Start ipsec for updates to take effect
ipsec start

# Prevent the container from exiting
while true; do
    sleep 60
done
